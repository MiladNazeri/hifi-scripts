...
(function() { 
    ...
    this.maybeUpdate = function() {
        ...
        var distance;
        if (MyAvatar.audioListenerMode === MyAvatar.audioListenerModeCustom) {
            distance = Vec3.length(Vec3.subtract(MyAvatar.customListenPosition, center));
        } else {
            distance = Vec3.length(Vec3.subtract(MyAvatar.position, center));
        }
        if (distance <= range) {
