...
(function(){
    ...
    var userDataArray = [
        ...
        makeFS(urls[8], channels[1], uniform[1]),
    ...
    var tweenArrays = groupProps.map(function(entity){
        ...
        var multiply = Vec3.multiply(qByV, 4);
        log("qbyV", multiply);
        var positionToMoveTo = Vec3.sum(entity.position, mkVFA([100,10,100]))
...
function midiEventReceived(eventData) {
    ...
    if (eventData.note == novationMap.knob_2){
        var newLength = lerp(0,127,1,10,eventData.velocity)
        changeLength(newLength);
    }
    if (eventData.note == novationMap.knob_3){
        var newWidth = lerp(0,127,1,10,eventData.velocity)

        changeWidth(newWidth);
    }
    if (eventData.note == novationMap.knob_4){
        var newHeight = lerp(0,127,1,10,eventData.velocity)
