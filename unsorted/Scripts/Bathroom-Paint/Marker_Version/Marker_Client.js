(function () {
    ...
    function onButtonClicked() {
        // console.log("isFingerPainting: ", isFingerPainting);
        var wasFingerPainting = isFingerPainting;
        // console.log("wasFingerPainting : ", isFingerPainting);
    ...
    function onEraseClosestLine(position){
        ...
        for (i = 0, entitiesLength = entities.length; i < entitiesLength; i += 1) {
            ...
            if (properties.type === "PolyLine") {
                polylines.push(entities[i]);
            }
        }
        var position = JSON.stringify(position);
        polylines = polylines.join(",");
        Entities.callEntityServerMethod(entityID, "eraseClosestLine", [position, polylines]);
    }
    ...
    BathroomMarkerClient.prototype = {
        ...
        startNearGrab: function () {
            console.log("### Start Near Grab CALLED");
            onButtonClicked();
        
        },
        startEquip: function(){
            console.log("@@@ Start Equip CALLED");
            
            onButtonClicked();
        },
        releaseEquip: function () {
            console.log("$$$ RELEASE EQUIP CALLED");
