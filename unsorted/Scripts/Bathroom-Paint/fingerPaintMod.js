...
(function () {
    var tablet,
        ...
        var fingerPaintChannel = "Finger-Entity-Channel";        
        ...
        function findName(name){
            ...
            entities.forEach(function(id){
                var entityName = Entities.getEntityProperties(id).name;
                if (entityName === name){
                    console.log("found bathrrom", id);
        ...
        var bathroomId = findName("Bathroom Zone");
    ...
    function paintBrush(name) {
        ...
        function startLine(position, width) {
            ...
            // Entities.callEntityServerMethod(bathroomId, "addPaint", props);
            Messages.sendMessage(fingerPaintServerChannel, JSON.stringify({
                type: "addPaint",
                props: props
            }));
        ...
        Messages.subscribe(fingerPaintChannel);
        Messages.messageReceived.connect(onMessage);
        function onMessage(channel, message, sender){
            if (channel === fingerPaintChannel){
                var varNewValue;
                if (message.type === "ENTITYID"){
                    varNewValue = Number(message.value);
                    entityID = varNewValue;
                }
                if (message.type === "BASEPOSITION"){
                    varNewValue = JSON.parse(message.value);
                    basePosition = varNewValue;
                }
        ...
        function drawLine(position, width) {
            ...
            if (distanceToPrevious >= MIN_STROKE_LENGTH
                ...
                // Entities.callEntityServerMethod(bathroomId, "editPaint", [entityID, props]);
                Messages.sendMessage(fingerPaintServerChannel, JSON.stringify({
                    type: "editPaint",
                    entityID: entityID,
                    props: props
        ...
        function finishLine(position, width) {
            ...
            if (strokePoints.length === 1) {
                // Delete "empty" line.
                // Entities.callEntityServerMethod(bathroomId, "deletePaint", [entityID]);
                Messages.sendMessage(fingerPaintServerChannel, JSON.stringify({
                    type: "deletePaint",
                    entityID: entityID
                }));
        ...
        function eraseClosestLine(position) {
            ...
            // Entities.callEntityServerMethod(bathroomId, "deleteClosest", [JSON.stringify(position), JSON.stringify(ERASE_SEARCH_RADIUS)]);
            Messages.sendMessage(fingerPaintServerChannel, JSON.stringify({
                type: "deleteClosest",
                position: position,
                ERASE_SEARCH_RADIUS: ERASE_SEARCH_RADIUS
            }));
